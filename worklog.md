...
---
Task ID: AI-Provider-Integration
Agent: main
Task: دمج مزود ذكاء اصطناعي خارجي (OpenAI-compatible) بدلاً من z-ai-web-dev-sdk

Work Log:
- إنشاء `src/lib/ai-client.ts` - عميل AI موحد يدعم أي مزود متوافق مع OpenAI API
- إضافة إعدادات المزود في `.env` (AI_PROVIDER_BASE_URL, AI_PROVIDER_API_KEY, AI_PROVIDER_MODEL)
- تحديث مسار `/api/ai/chat` (المفكر القانوني) لاستخدام العميل الجديد
- تحديث مسار `/api/ai/analyze` (محلل النصوص) لاستخدام العميل الجديد
- تحديث مسار `/api/ai/pleading` (مساعد المرافعة) لاستخدام العميل الجديد
- تحديث مسار `/api/ai/generate-doc` (توليد المستندات) لاستخدام العميل الجديد
- إنشاء مسار `/api/ai/provider` لإدارة إعدادات المزود (GET/POST/PUT للاختبار)
- إضافة تبويب "الذكاء AI" في واجهة الإعدادات مع:
  * عرض حالة الاتصال
  * حقول Base URL و API Key و Model
  * زر "اختبار الاتصال"
  * زر "حفظ الإعدادات"
  * معلومات توضيحية
- إضافة برومبتات النظام الجاهزة في ai-client.ts (4 برومبتات متخصصة)
- اختبار شامل لجميع الميزات الأربعة عبر curl

Stage Summary:
- ✅ المزود الخارجي يعمل: https://api.freemodel.dev/v1 مع نموذج gpt-5.5
- ✅ المفكر القانوني يرد بالعربية الفصحى بأسلوب قانوني احترافي
- ✅ محلل النصوص يكشف الثغرات القانونية في العقود
- ✅ توليد المستندات ينتج عقود قانونية كاملة بالهيكل الصحيح
- ✅ مساعد المرافعة يولّد هيكل مرافعة متكامل
- ✅ اختبار الاتصال يعمل من واجهة الإعدادات
- ✅ يمكن للمستخدم تغيير المزود/المفتاح/النموذج من الإعدادات
- ✅ باقي أقسام النظام تعمل محلياً 100% بدون إنترنت
